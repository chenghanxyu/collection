$(window).load(function(){
		...
		var mouse = {x:0 , y:0};
		 ...
		 var   arrayactualy = [];
		 var   ml , mr , nt , nb;
		 var   top = 0 , left = 0 , num=0 , min = 2 ,   sec = 5 , empty;
 				...
 				for(var i=0; i<arraynow.length ;i++){
 					if(arraynow[i].left == arrayactualx[i] && arraynow[i].top == arrayactualy[i] ){
 						...
 						if(num==9){
								...
								swal({
                  title: '恭喜你完成拼圖！',
                  text: "快點擊下方按鈕索取《保持沉默》電影觀影券",
                  icon: "success",
                  button: "立即索取",
                }).then(function(){window.location.href= 'https://h5.smartcinema.com.tw/ticket-intl-tw/index.html?giveCode=L2RLR8QO&source=&_lang=zh_TW'});
							  flag = false;
 						}
 					}
 				}
  ...
  //                 icon: "warning",
  //                 button: "重新挑戰",
  //               }).then(function(){window.location.href= 'https://smartcinematw.neocities.org/puzzle/'});
 	// 			}
 	// 		}
 	// 	}
